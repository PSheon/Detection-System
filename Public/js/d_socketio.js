...
$(document).ready(function(){
  ...
  socketio.on('Atten_emit', function(data) {
    ...
    $('#focus_bar').css('width', atten+'%');
    $('#focus_bar').text(atten+' %');
    if(atten > 40) {
      $('#focus_bar').attr('class', 'progress-bar progress-bar-success');
    }else if(atten > 10 ) {
      $('#focus_bar').attr('class', 'progress-bar progress-bar-warning');
    }else if(atten > 0 ) {
      $('#focus_bar').attr('class', 'progress-bar progress-bar-danger');
    }
    console.log('attention : '+atten);
  });
  socketio.on('Meditate_emit', function(data) {
    var meditate = data.Meditate;
    $('#relax_bar').css('width', meditate+'%');
    $('#relax_bar').text(meditate+' %');
    if(meditate > 40) {
      $('#relax_bar').attr('class', 'progress-bar progress-bar-success');
    }else if(meditate > 10 ) { 
      $('#relax_bar').attr('class', 'progress-bar progress-bar-warning');
    }else if(meditate > 0 ) { 
      $('#relax_bar').attr('class', 'progress-bar progress-bar-danger');
    }
  ...
  socketio.on('Blink_emit', function(data) {
    ...
    blink = (blink/200)*100;
    $('#blinkeye_bar').css('width', blink+'%');
    $('#blinkeye_bar').text(blink+' %');
    $('#blinkeye_bar').attr('class', 'progress-bar progress-bar-success');
